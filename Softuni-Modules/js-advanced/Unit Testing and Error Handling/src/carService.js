const carService = {
  ...
  discount(numberOfParts, totalPrice) {
    if (typeof numberOfParts !== "number" || typeof totalPrice !== "number") { // done 
      throw new Error("Invalid input"); 
    }

    let discountPercentage = 0;

    if (numberOfParts > 2 && numberOfParts <= 7) { // done
      discountPercentage = 15;
    } else if (numberOfParts > 7) { // done
    ...
    if (numberOfParts <= 2) {
      return "You cannot apply a discount"; // done 
    } else {
      return `Discount applied! You saved ${result}$`; // done
    }
  },
  partsToBuy(partsCatalog, neededParts) {
    let totalSum = 0;

    if (!Array.isArray(partsCatalog) || !Array.isArray(neededParts)) { // done
      throw new Error("Invalid input");
    }
